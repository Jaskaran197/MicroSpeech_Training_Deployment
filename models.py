...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_dnn_model(fingerprint_input, model_settings,
                              is_training)
  else:
...
def create_dnn_model(fingerprint_input, model_settings, 
                       is_training):
  ...
  for i in range(1, num_layers + 1):
      with tf.variable_scope('fc'+str(i)):
          W = tf.compat.v1.get_variable('weights', shape=[layer_dim[i-1], layer_dim[i]], 
                initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.001)),
...
def create_dnn_model(fingerprint_input, model_settings, 
  ...
  weights = tf.compat.v1.get_variable('final_fc', shape=[layer_dim[-1], label_count], 
              initializer=tf.initializers.GlorotUniform())
  bias = tf.compat.v1.get_variable(name='final_bias', shape=[label_count])
  logits = tf.matmul(flow, weights) + bias
  if is_training:
    return logits, dropout_prob
  else:
